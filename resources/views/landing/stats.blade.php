...
<section id="statistik" class="relative py-24 sm:py-32 overflow-hidden">
    <div class="absolute inset-0 bg-slate-950 -z-20"></div>

    {{-- Mesh gradient background --}}
    <div class="absolute inset-0 -z-10">
        <div class="absolute top-0 left-0 right-0 bottom-0 bg-gradient-to-br from-brand-950 via-slate-900 to-violet-950"></div>
    ...
    <div class="mx-auto max-w-7xl px-5 sm:px-6 lg:px-8">
        ...
        <div class="lp-reveal mx-auto max-w-2xl text-center mb-16">
            <span class="inline-block rounded-full bg-white/5 border border-white/10 px-4 py-1.5 text-xs font-bold uppercase tracking-widest text-slate-400 mb-5">
                Pencapaian
            </span>
            ...
            <p class="mt-5 text-slate-400">Data nyata dari seeder awal — tumbuh seiring penggunaan sistem di sekolah Anda.</p>
        </div>
        ...
        <div class="lp-reveal grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            @php
                $stats = [
                    ['168', 'Siswa Terdaftar', 'siswa', 'brand', '+ peserta didik aktif'],
                    ['24',  'Tenaga Pengajar', 'guru',  'emerald', 'guru & staf berdedikasi'],
                    ['8',   'Rombongan Belajar', 'kelas', 'amber', 'kelas berbagai jenjang'],
            ...
            @foreach ($stats as $i => [$value, $label, $icon, $color, $sub])
                <div class="lp-reveal text-center" style="--lp-delay: {{ $i * 100 }}ms"
                    ...
                    <div class="relative inline-flex flex-col items-center rounded-3xl border border-white/10 bg-white/5 px-8 py-10 backdrop-blur-sm w-full">
                        <div class="absolute -top-px left-1/2 -translate-x-1/2 h-px w-24 bg-gradient-to-r from-transparent via-{{ $color }}-400/60 to-transparent"></div>
                        <div class="mb-4 flex h-14 w-14 items-center justify-center rounded-2xl bg-{{ $color }}-500/15 text-{{ $color }}-400">
                            <x-icon name="{{ $icon }}" class="h-7 w-7" />
                        </div>
                        <div class="text-5xl font-black tabular-nums text-white sm:text-6xl">
                            <span x-text="shown.toLocaleString('id-ID')">0</span>{{ str_contains($value, '+') ? '+' : '' }}
                        </div>
                        <div class="mt-2 text-base font-bold text-{{ $color }}-400">{{ $label }}</div>
        ...
        <div class="lp-reveal mt-16 text-center">
            <blockquote class="relative mx-auto max-w-2xl">
                ...
                <p class="text-lg italic text-slate-300 leading-relaxed">
