...
class WhatsappWebhookController extends Controller
{
    ...
    /**
     * Endpoint webhook untuk menerima pesan masuk dari WhatsApp Gateway.
     */
    public function handle(Request $request): JsonResponse
    {
        $rawContent = $request->getContent();
        Log::info('[WhatsApp Webhook] Payload Received: ' . $rawContent);
        ...
        if ($secret !== '') {
            ...
            if (! hash_equals($expected, $signature)) {
                Log::warning('[WhatsApp Webhook] Signature mismatch — ditolak.');
        ...
        if (! $sender || ! $message) {
            Log::warning('[WhatsApp Webhook] Missing sender/message: sender=' . json_encode($sender) . ' message=' . json_encode($message));
        ...
        try {
            ...
            Log::error('[WhatsApp Webhook] Chatbot error: ' . $e->getMessage(), [
